...
+ [新增] 渠道兼容Gemini格式。
+ [新增] 新增Codex App插件支持。
+ [新增] 新增网页自助注册、账号密码登录和个人账号配置修改；账号运行数据写入 data/auth-users.json，画布、素材和生成记录仍保存在浏览器本地。
+ [新增] 提示词库新增 songtianlun/awesome-prompts 数据源（138 条带图案例）。
+ [调整] 品牌定制：项目名称改为 BMCCA无限画布，更换 Logo 和网站图标。
+ [调整] Base URL 锁定为 https://cca.maya.today/ ，用户不可修改。
+ [调整] 渠道名称固定前缀 BMCCA + 自定义后缀，不再自动编号。
+ [调整] 注册和账号配置合并"用户名"和"显示名称"为单个字段。
+ [调整] 移除首页 GitHub 链接和文档入口图标，放大顶栏图标和品牌文字。
+ [修复] 修复 canvas 页面 useSearchParams 未用 Suspense 包裹导致构建失败。
+ [修复] 移除旧 favicon.ico，解决浏览器标签栏图标不更新问题。
+ [修复] 首页提示词过滤无封面图条目，避免 broken image。
+ [调整] 默认端口从 3000 改为 30026，避免端口冲突。
+ [调整] 默认渠道模型列表只保留 gpt-image-2，过滤其他模型。
